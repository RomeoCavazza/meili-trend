...
async function search() {
  ...
  if (!data.hits?.length) {
    res.innerHTML = `<li class="muted">Aucun résultat<br><button onclick="ingest('${q}')">Ingérer #${q}</button></li>`;
    return;
  }
...
async function ingest(tag) {
  const res = $("#results");
  res.innerHTML = `<li class="muted">⏳ Ingestion...</li>`;
  ...
  const data = await r.json();
  res.innerHTML = `<li class="muted">✅ ${data.inserted} posts</li>`;
